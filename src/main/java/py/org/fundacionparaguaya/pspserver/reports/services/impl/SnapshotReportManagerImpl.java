...
public class SnapshotReportManagerImpl implements SnapshotReportManager {
  ...
  private static final List<String> DEFAULT_HEADERS =
      Arrays.asList("organizationName", "familyCode", "familyName", "createdAt");
  ...
  public SnapshotReportManagerImpl(FamilyRepository familyRepository,
      FamilyDTOMapper familyReportMapper, SnapshotEconomicRepository snapshotRepository,
      SnapshotIndicatorMapper snapshotMapper, SurveyRepository surveyRepository, I18n i18n,
  ...
  public List<OrganizationFamilyDTO> listFamilyByOrganizationAndCreatedDate(
    ...
    Specification<FamilyEntity> dateRange =
        FamilySpecification.createdAtBetween2Dates(filters.getDateFrom(), filters.getDateTo());

    families = familyRepository.findAll(where(byOrganization(filters.getOrganizationId()))
        .and(dateRange).and(byApplication(filters.getApplicationId())).and(dateRange), sort);

    Map<OrganizationEntity, List<FamilyEntity>> groupByOrganization =
        families.stream().filter(f -> f != null && f.getOrganization() != null)
    ...
    groupByOrganization.forEach((k, v) -> {
      OrganizationFamilyDTO fa =
          new OrganizationFamilyDTO(k.getName(), k.getCode(), k.getDescription(), k.isActive());
  ...
  public List<FamilySnapshotDTO> listSnapshotByFamily(SnapshotFilterDTO filters) {
    ...
    if (filters.getDateFrom() != null && filters.getDateTo() != null
      ...
      Map<SurveyEntity, List<SnapshotEconomicEntity>> groupBySurvey =
          snapshots.stream().collect(Collectors.groupingBy(s -> s.getSurveyDefinition()));

      groupBySurvey.forEach((k, v) -> {

        FamilySnapshotDTO familySnapshots =
            new FamilySnapshotDTO(filters.getFamilyId(), k.getTitle());
    ...
    if (filters.getDateFrom() != null && filters.getDateTo() != null) {
      ...
      snapshots = snapshotRepository.findAll(
          where(SnapshotEconomicSpecification.byApplication(filters.getApplicationId()))
              .and(dateRange)
              .and(SnapshotEconomicSpecification.byOrganizations(filters.getOrganizationId())),
          sort);
    }
  ...
  private List<String> getSortedKeys(SurveyEntity survey) {
    ...
    List<String> personalInformationKeys =
        survey.getSurveyDefinition().getSurveyUISchema().getGroupPersonal();
    List<String> socioEconomicsKeys =
        survey.getSurveyDefinition().getSurveyUISchema().getGroupEconomics();
    List<String> indicatorsKeys =
        survey.getSurveyDefinition().getSurveyUISchema().getGroupIndicators();
  ...
  private List<List<String>> getRows(SurveyEntity survey, List<SnapshotEconomicEntity> snapshots,
      List<String> keys) {
    List<SurveyData> rows = new ArrayList<>();
    ...
    for (SnapshotEconomicEntity snapshot : snapshots) {
      ...
      if (snapshot.getFamily() != null) {
        data.put(i18n.translate("snapshot.report.header.familyName"),
            snapshot.getFamily().getName());
        data.put(i18n.translate("snapshot.report.header.familyCode"),
            snapshot.getFamily().getCode());
        if (snapshot.getFamily().getOrganization() != null) {
          ...
          data.put(i18n.translate("snapshot.report.header.familyCode"),
              "ORG. NAME: " + snapshot.getFamily().getOrganization().getName() + ". FAMILY CODE: "
                  + snapshot.getFamily().getCode());
        }
      }
      data.put(i18n.translate("snapshot.report.header.createdAt"),
          snapshot.getCreatedAtLocalDateString());
  ...
  private String reportToCsv(ReportDTO report) {
    ...
    for (List<String> row : report.getRows()) {
      toRet =
          toRet + (row.stream().map(Object::toString).collect(Collectors.joining(CSV_DELIMITER)))
              .concat("\n");
    }
    return toRet;
  }
